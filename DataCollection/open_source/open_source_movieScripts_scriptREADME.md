...
Before running a long scrape, you can audit the current source coverage:

```bash
python audit_sources.py
```

This reports which sources are still live, which ones appear dead or repurposed, and where your local metadata or file counts lag the currently visible source indexes. You can also save the report:

```bash
python audit_sources.py --json-out scripts/metadata/source_audit.json
```
